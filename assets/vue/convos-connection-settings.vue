<template>
  <form autocomplete="off" class="content" @submit.prevent>
      ...
      <div class="col s12">
        ...
        <p v-if="!connection">
          ...
          <template v-if="!user.connections.length && settings.default_server">
            In most cases, you can just hit <i>"Create"</i> get started.
          </template>
        </p>
      </div>
    </div>
    <template v-if="connection">
      <div class="row">
        <md-input :value.sync="server" :readonly="settings.forced_irc_server" cols="s12">Server</md-input>
      </div>
      <div class="row">
        <md-input :value.sync="nick">Nick</md-input>
      </div>
      <div class="row">
        <div class="col s12">
    ...
    <template v-if="!connection">
      <div class="row">
        <md-input :value.sync="server" :readonly="settings.forced_irc_server" cols="s6">Server</md-input>
        <md-input :value.sync="nick" cols="s6">Nick</md-input>
      </div>
    </template>
    <div class="row">
      <div class="col s12">
    ...
    <template v-if="advancedSettings">
      ...
      <div class="row on-connect-commands">
        <md-textarea v-ref:occ :value.sync="onConnectCommands">On Connect Commands (one per line)</md-textarea>
      </div>
    </template>
  ...
  methods: {
    ...
    saveConnection: function() {
      ...
      connection.save(function(err) {
        ...
        if (self.settings.main.indexOf(this.connection_id) == -1) {
          self.settings.main = "#chat/" + this.connection_id + "/" + this.connection_id;
        }
      });
    },
    updateForm: function() {
